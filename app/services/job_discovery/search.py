...
def _exact_url_deduplicate(jobs: pd.DataFrame) -> pd.DataFrame:
    if jobs.empty:
        return jobs.copy()
    updated = jobs.copy()
    urls = updated["job_url"].fillna("").astype(str).str.strip().str.lower()
    with_url = updated[urls != ""].drop_duplicates(
        subset=["job_url"],
        keep="first",
    )
    without_url = updated[urls == ""]
    return pd.concat(
...
def search_jobs(
    ...
    try:
        ...
        raise RuntimeError(
            f"Job search failed for '{search_term}': {exc}"
        ) from exc

    if jobs is None or jobs.empty:
        return pd.DataFrame(columns=DISPLAY_COLUMNS)

    return _exact_url_deduplicate(
        _ensure_columns(
            _normalise_location(jobs)
        )
    )
