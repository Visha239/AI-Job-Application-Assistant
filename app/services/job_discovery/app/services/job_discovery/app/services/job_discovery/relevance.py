...
def _is_remote(row: dict[str, Any]) -> bool:
    ...
    return "remote" in location or "work from home" in description
...
def _contains_seniority(title: str) -> bool:
    ...
    return any(word in words for word in SENIORITY_WORDS)
...
def evaluate_relevance(
    ...
    required_experience = extract_required_experience_years(combined)

    if required_experience is None:
        experience_score = 10
        reasons.append("No clear high experience requirement was detected.")
        ...
        experience_score = 8
        warnings.append(
            f"Role may stretch the profile at {required_experience}+ years."
        )
    else:
        experience_score = 0
        relevance_penalty += 30
        warnings.append(
            f"Role asks for around {required_experience}+ years."
        )

    if (
        "bengaluru" in location
        or "bangalore" in location
        or "karnataka" in location
    ):
        location_score = 15
