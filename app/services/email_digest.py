...
def validate_email_settings(
    ...
    if not settings.app_password:
        errors.append("Gmail app password is missing.")

    if (
        not settings.recipient_email
        or "@" not in settings.recipient_email
    ):
...
def build_digest_email(
    ...
    message.set_content(
        text_summary
        or "Your CareerPilot daily job digest is ready."
    )
...
def send_html_email(
    ...
    if errors:
        raise ValueError(
            "Email configuration error: " + " ".join(errors)
        )
    ...
    try:
        ...
        raise RuntimeError(
            "Gmail authentication failed. Check the email address "
            "and use the generated 16-character App Password."
        ) from exc
